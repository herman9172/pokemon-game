...
import { AuthenticationTokens, IAuthenticationRepository, IAuthenticationRequest, IAuthenticationTokens, ICacheRepository,  ILogRepository, IRequestRepository } from '@core/interface_adapters/adapters';
...
export class AuthenticationRepository implements IAuthenticationRepository {
  ...
  async getAuthenticationTokens(authenticationRequest: IAuthenticationRequest): Promise<AuthenticationTokens> {
    ...
    if (!authenticationToken.accessToken) {
      const base64Credentials = Buffer.from(`${authenticationRequest.clientId}:${authenticationRequest.secretKey}`).toString('base64');
